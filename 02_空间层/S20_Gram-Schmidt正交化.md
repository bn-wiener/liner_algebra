# S20 Gram–Schmidt 正交化

> **学习定位**：本节属于第二层“空间层——有什么”。正文重点回答“哪些向量可以互相表示、存在多少个独立方向、这些方向形成什么空间”。涉及矩阵作用时只给必要的变换直觉，正式的线性变换理论留到第三层。

## 本节从什么问题产生

一般的线性无关向量组可以作为基，但不方便做投影和坐标提取。我们希望在**不改变张成空间**的前提下，把它变成标准正交基。

## Gram–Schmidt 的完整推导、算法与例子
...
Gram–Schmidt 的核心动作是：处理一个新向量时，逐个减去它在已有正交方向上的投影，留下与旧方向都正交的新成分，再单位化。整个过程改变基的形式，但保持张成空间不变。若中途得到零向量，说明原输入向量组线性相关。

## 下一步

现在我们能方便地做投影。下一步回到 $A\boldsymbol x=\boldsymbol b$：如果 $\boldsymbol b$ 根本不在列空间里，就把它投影到列空间，从“无解”变成“最佳近似”。
